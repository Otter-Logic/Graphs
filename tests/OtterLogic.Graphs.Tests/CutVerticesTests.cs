...
public class CutVerticesTests
{
    ...
    /// <summary>
    /// The edge between the triangle and the square is the only bridge: every other
    /// edge lies on a cycle. The cut vertices are its two ends.
    /// </summary>
    [Fact]
    public void ABridgeIsAnEdgeOnNoCycle()
    {
        var graph = WeightedGraph.FromEdges(7, new[]
        {
            (0, 1), (1, 2), (2, 0),
        ...
        var cuts = CutVertices.Of(graph);

        Assert.Equal(new[] { (2, 3) }, cuts.Bridges);
        Assert.Equal(new[] { 2, 3 }, cuts.ArticulationPoints());
        Assert.Equal(CutVertices.Stranded(graph), cuts.Stranded);
    }

    /// <summary>
    /// Every edge of a tree is a bridge, reported lower node first in ascending
    /// order whatever order the search met them in; a cycle has none.
    /// </summary>
    [Fact]
    public void EveryEdgeOfATreeIsABridgeAndNoEdgeOfACycleIs()
    {
        var tree = WeightedGraph.FromEdges(6, new[] { (4, 1), (1, 0), (0, 3), (3, 5), (3, 2) });
        Assert.Equal(new[] { (0, 1), (0, 3), (1, 4), (2, 3), (3, 5) }, CutVertices.Of(tree).Bridges);

        var cycle = WeightedGraph.FromEdges(4, new[] { (0, 1), (1, 2), (2, 3), (3, 0) });
        Assert.Empty(CutVertices.Of(cycle).Bridges);
    }
}
