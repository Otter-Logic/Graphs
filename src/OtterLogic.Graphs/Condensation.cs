...
public sealed record CondensationResult(int[] Component, int ComponentCount, int[] Height)
{
    ...
    /// <summary>Whether no two nodes depend on each other — every component a single node.</summary>
    public bool IsAcyclic => ComponentCount == Component.Length;

    /// <summary>
    /// The nodes with everything a node depends on placed before it: a topological
    /// order, lowest height first and by index within a height.
    /// <para>
    /// No second algorithm. An arc from one component to another always drops at
    /// least one in height, so sorting by height already puts every head before its
    /// tail, and sorting by index within a height makes the order the graph's
    /// rather than the search's. Nodes on a cycle share a height and come out side
    /// by side, which is as much of an order as a cycle has; check
    /// <see cref="IsAcyclic"/> where a strict one is required.
    /// </para>
    /// </summary>
    public int[] TopologicalOrder()
        => Enumerable.Range(0, Component.Length).OrderBy(HeightOf).ThenBy(i => i).ToArray();
}
...
{
    /// <summary>Folds the cycles of <paramref name="graph"/> and ranks what is left. Weights play no part.</summary>
    public static CondensationResult Of(DirectedGraph graph)
    {
        ArgumentNullException.ThrowIfNull(graph);
        return Of(graph.NodeCount, graph.Arcs().Select(arc => (arc.From, arc.To)));
    }
