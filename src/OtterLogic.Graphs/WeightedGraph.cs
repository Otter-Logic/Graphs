...
/// <para>
/// It lives below every layer that uses it because it is a data contract, not an
/// algorithm. Spectral clustering, connectivity-constrained hierarchical
/// clustering and message passing in Unsupervised all consume it, a trained graph
/// network in DeepLearning will want exactly the same thing on its input side —
/// the same edges, the same normalisation — and a toolkit routing a toolpath or
/// tracing a load path wants it with no machine learning in sight. One type below
/// all of them is what stops four slightly different adjacency lists appearing.
/// </para>
/// <para>
...
/// from that; a caller with only a point cloud uses MachineLearning's
/// <c>NeighbourGraph.Of</c>, which sits up there because measuring a distance
/// between samples is that layer's business and nothing here knows what a sample
/// is. The algorithms above cannot tell the difference, which is the point.
/// </para>
/// <para>
...
/// </para>
/// </summary>
public sealed class WeightedGraph : IAdjacency
{
    ...
    /// <summary>
    /// The same graph with every edge as two arcs, one each way, at the edge's weight.
    /// <para>
    /// Lossless, so it asks nothing. It is how a network that is mostly two-way
    /// gets its few one-way streets: start from this and rebuild with the arcs that
    /// do not apply left out.
    /// </para>
    /// </summary>
    public DirectedGraph ToDirected()
        => DirectedGraph.FromArcs(NodeCount,
            Edges().SelectMany(e => new[] { (e.A, e.B, e.Weight), (e.B, e.A, e.Weight) }),
            DuplicateArcs.KeepLargest);

    ReadOnlySpan<int> IAdjacency.Next(int node) => Neighbours(node);
    ReadOnlySpan<double> IAdjacency.NextWeights(int node) => EdgeWeights(node);
